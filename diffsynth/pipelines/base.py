...
class BasePipeline(torch.nn.Module):
    ...
    def to(self, *args, **kwargs):
        """Override to() to also update torch_dtype when dtype is specified."""
        result = super().to(*args, **kwargs)
        # Update torch_dtype if dtype is specified
        if 'dtype' in kwargs:
            self.torch_dtype = kwargs['dtype']
        else:
            # Check if dtype is passed as positional argument
            for arg in args:
                if isinstance(arg, torch.dtype):
                    self.torch_dtype = arg
                    break
        return result
